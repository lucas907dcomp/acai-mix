...
function PriceCard() {
  ...
  useEffect(() => {
    if (product) reset({ price_per_gram: product.price_per_gram })
  }, [product, reset])
  ...
  async function onSubmit(values: PriceForm) {
    ...
    const { error } = await supabase
      .from('products')
      .update({ price_per_gram: values.price_per_gram })
      .eq('id', product.id)
    if (error) {
        ...
        {isLoading ? (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="space-y-1.5">
              <label className="text-sm text-[#9d7bc8]" htmlFor="price_per_gram">
                Preço por grama (R$/g)
              </label>
              <input
                id="price_per_gram"
              ...
              {errors.price_per_gram && (
                <p className="text-red-400 text-xs">{errors.price_per_gram.message}</p>
              )}
              <p className="text-[#9d7bc8] text-xs">
                Ex: 0,0650 = R$0,0650 por grama (R$65,00/kg)
              </p>
            </div>
            <div className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => reset({ price_per_gram: product?.price_per_gram })}
                disabled={!isDirty}
